...
def ctc_loss(pred, pred_len, token, token_len, blank=0):
    ...
    # token_with_blank
    token_with_blank = T.cat((T.zeros(batch, U, 1).type(longX), token[:, :, None]), dim=2).view(batch,
                                                                                                -1)  # (batch, 2U)
    token_with_blank = T.cat((token_with_blank, T.zeros(batch, 1).type(longX)), dim=1)  # (batch, 2U+1)
    length = token_with_blank.size(1)

    pred = pred[
        T.arange(0, Time).type(longX)[:, None, None], T.arange(0, batch).type(longX)[None, :, None], token_with_blank[
                                                                                                     None,
                                                                                                     :]]  # (T, batch, 2U+1)

    # recurrence relation
    sec_diag = T.cat(
        (T.zeros((batch, 2)).type(floatX), T.ne(token_with_blank[:, :-2], token_with_blank[:, 2:]).type(floatX)),
        dim=1) * T.ne(token_with_blank, blank).type(floatX)  # (batch, 2U+1)
    recurrence_relation = (m_eye(length) + m_eye(length, k=1)).repeat(batch, 1, 1) + m_eye(length, k=2).repeat(batch, 1,
                                                                                                               1) * sec_diag[
                                                                                                                    :,
                                                                                                                    None,
                                                                                                                    :]  # (batch, 2U+1, 2U+1)
...
def test_seg_ctc1(use_mine=True):
    # (T, voca_size+1)
    pred_np = np.array([[0.5, 0.4, 0.1], [0.3, 0.1, 0.6], [0.7, 0.2, 0.1], [0.3, 0.5, 0.2]])[:, None]
    pred_np = np.log(np.tile(pred_np, (1, 2, 1)))
    # (U)
    ...
    token_len_np = np.random.randint(low=29, high=30, size=n)
    ...
    for i in range(100):
        cost = ctc_cost(pred, token, sizes, target_sizes)
        print(cost.data.item() / 29.0)

        # from warpctc_pytorch import CTCLoss
        criterion = T.nn.CTCLoss(blank=0, reduction='none')
        cost1 = criterion(pred, token, sizes, target_sizes)
        print("CTC {}".format(cost1.item()))
