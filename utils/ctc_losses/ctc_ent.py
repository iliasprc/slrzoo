...
from utils.ctc_losses.m_ctc import m_eye, log_batch_dot, log_sum_exp
...
def ctc_ent_loss(pred, pred_len, token, token_len, blank=0):
    ...
    beta_t = T.cat((pred[0, :, :2] * T.log(pred[0, :, :2]), T.zeros(batch, 2 * U - 1).type(floatX)),
                   dim=1)  # (batch, 2U+1)
...
def ctc_ent_cost(out, targets, sizes, target_sizes):
