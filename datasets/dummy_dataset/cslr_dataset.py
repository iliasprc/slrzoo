...
class RandomCSLRdataset(BaseDataset):
    def __init__(self, config, args, mode, classes):
        config = OmegaConf.load(os.path.join(args.cwd, "datasets/dummy_dataset/dataset.yml"))['dataset']
        super(RandomCSLRdataset, self).__init__(config, args, mode, classes)
    ...
    def __getitem__(self, idx):
        ...
        video = torch.rand(*self.video_size)

        # Generate a random target label
        target = torch.tensor([random.randint(0, self.num_classes - 1),random.randint(0, self.num_classes - 1)])
